...
flowchart
  ...
  subgraph 業務モデル
    ...
    ビジネスルール[060-ビジネスルール]
    業務イベント一覧[080-業務イベント一覧]
    業務受け入れ条件[100-業務受け入れ条件]
  end

  subgraph インターフェースモデル
    画面仕様[070-画面仕様]
    システム化機能一覧[090-システム化機能一覧]
  end
...
| 070-画面仕様           | 業務ユーザー視点の画面要件を定義                 | 画面一覧、目的、項目一覧、入力制約、ボタン動作、メッセージ | 「画面：在庫一覧／項目：商品コード、商品名、在庫数、発注点、在庫状態…」                 |
...
| 110-用語集             | 用語の意味を統一する                             | 用語、定義、備考、類似語・禁止語                           | 「発注点：在庫がこの数量を下回ったときに発注候補とみなす基準数量。」                    |
