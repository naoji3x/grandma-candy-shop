...
> - 対象は 1 つのビジネスルール（`br-xxx-xxxx`）です。システム実装詳細（クラス名・SQL・画面操作手順など）は書かず、**業務レベルの判断/計算**に限定してください。
...
>   ```
>
> - **入力** は概念レベルの業務データで記述し、業務データ辞書にあるエンティティ・論理名を想定して書いてください。物理項目・SQLは絶対に書かないでください。
