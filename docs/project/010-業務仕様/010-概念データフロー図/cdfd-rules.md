# 概念データフロー図（Conceptual Data Flow Diagram: CDFD）ドキュメント作成ルール

本ドキュメントは、業務分析・要求定義のために **概念データフロー図（Conceptual Data Flow Diagram: CDFD）** を、統一形式で作成するためのルールです。

この章の成果物は次の2つをセットで扱います。

- **プロセス概要表**: 主要プロセスを表で合意する（言葉の定義・責務境界の合意）
- **業務フロー（Mermaid）**: プロセス間の「情報の流れ」「物の流れ」「イベント」を図で合意する（流れの合意）

Mermaid 記法そのもののルールは [cdfd-mermaid-rules.md](cdfd-mermaid-rules.md) を参照してください。
...
- プロセス概要表に記載したプロセスは、対応する図にも登場させます（省略する場合は理由を補足）。
...
>   - `凡例`は [cdfd-mermaid-rules.md](cdfd-mermaid-rules.md)を参照するように記載ください。
