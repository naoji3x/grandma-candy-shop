...
以下は、**Mermaid の `flowchart` 構文を使って DFD（データフロー図）を描く際の標準ルール**です。
生成 AI に DFD を描かせるときの「指示書」として利用します。
ここでは **ノードIDも日本語** を用いるものとします。
...
| 物理保管場所     | 商品の物理的な保管場所（棚など） | 円柱: `在庫棚([在庫棚])`           |
...
- **角丸長方形 `()`** を使う。
- ノードIDは日本語でよい。ID と表示ラベルを同じにしてよい。
...
- 複数のプロセスの記載を省略する場合は、`...に関する複数プロセス<br>（詳細省略）`と記載して、１プロセスで表現する。
...
flowchart LR
  ...
  subgraph 入荷業務概要[入荷業務]
    入荷業務プロセス群(入荷に関する複数プロセス<br>（詳細省略）)
  end
```
...
flowchart LR
  subgraph 凡例[凡例]
      ...
      処理1 ==>|物| 外部主体 ==>|物| 処理4 ==>|物| 物理保管場所
  end
  classDef invisibleNode fill:none,stroke:none,color:inherit;
```
...
生成 AI に DFD を作らせるときは、以下のような指示を与える。

はい、その指示で概ね問題ありませんが、以下の点を追加・修正するとより明確になります：
...
> - プロセスグループ内の複数プロセスを省略する場合は、1つの角丸長方形ノードで `...に関する複数プロセス<br>（詳細省略）` のように記載してください（例: `入荷業務プロセス群(入荷に関する複数プロセス<br>（詳細省略）)`）。
...
このルールとテンプレートをコピーして、生成 AI のプロンプトに貼り付けて利用してください。なお、`mermaid-dfd-instruction.md`として別ファイルに保存しています。
