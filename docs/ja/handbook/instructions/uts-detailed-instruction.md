...
- UTS-D は **テストケースの具体値やケース列挙**をしません（詳細は UTD/テストコードへ）。
...
- 観点ID・条件IDは **この文書内で一意**であればOKです（例: `UT-I-01`、`UC-01`）。
- 他文書から参照する可能性がある場合は、`<docId>:<観点ID>`（例: `uts-inventory:UT-I-01`）のように完全修飾で扱えるようにしてください。
...
- 具体値の大量列挙は禁止です（必要なら“境界値”と書き、詳細は UTD に委ねる）。
