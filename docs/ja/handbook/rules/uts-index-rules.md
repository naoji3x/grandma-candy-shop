...
本ドキュメントは、単体テスト仕様（UTS）の全体構成 `uts-index` を統一形式で記述するためのルールです。
TPC の観点・条件を単体テストの責務として解釈し、個別仕様 `uts-<term>` へ分配する **入口（ナビゲーション）** を定義します。

個別単体テスト仕様（`uts-<term>`）の記述ルールは [uts-rules.md](uts-rules.md) を参照してください。

## 1. 目的

`uts-index` は以下を明文化し、単体テストの責務・境界・合格基準の認識ズレを防ぎます。
...
- `based_on` には最低限、関連する `tpc-...` を含めます。
  ...
  - BAC/NFR/BR 等は、`uts-index` で方針判断（境界・分配・共通合格基準）に **直接利用したものだけ**を列挙します（大量になる場合は TPC 側のトレース表に集約）。
- なお、個別仕様（`uts-<term>`）では、判断根拠の明確化のため `based_on` を必要十分に列挙します（最低限 `tpc-*` を含め、必要に応じて `br-*` / `bac-*` / `nfr-*` 等も含めます）。
...
対象単位（`uts-<term>`）の一覧は、**7.5 の「対象単位一覧（統合表）」** で管理します（同一内容の重複記載はしません）。
...
対象単位の定義・TPC の割当・個別仕様リンクは 7.5 の「対象単位一覧（統合表）」で一括管理します（重複記載しません）。

### 7.5. 対象単位一覧（統合表）（必須）

運用上の更新漏れを防ぐために、**対象単位の定義（7.2）・分配方針（7.4）・個別仕様一覧（7.7）を 1 表に統合**し、ここを単一ソース（Single Source of Truth）とします。

以下の表で、対象単位の定義・TPCの割当・個別仕様リンクを一括で管理します。

`uts-index` 本文では、見出し「5. 対象単位一覧（統合表）」としてこの表を配置します。
...
※ 実運用では、各 `uts-<term>` が指す個別仕様ファイル（例: `./uts-<term>.md`）へリンクしてください（実ファイル名・配置はプロジェクトに合わせます）。

### 7.6. 合格基準とエビデンス（共通）（必須）
...
- 個別仕様一覧は、**7.5 の「対象単位一覧（統合表）」の `uts-<term>` 列（リンク）**で管理します。
- 一覧の重複記載はせず、7.5 の統合表が最新版であることを保証します（命名ゆれ禁止）。
...
- 合否判定に影響する事項はここに置かず、本文（特に 7.4 / 7.6）へ反映します。
...
対象単位（`uts-<term>`）の一覧は、「対象単位一覧（統合表）」（9.6）で管理する。

### 9.4. 単体テストの対象範囲と境界（モック/スタブ方針）
...
個別仕様一覧は、9.6 の「対象単位一覧（統合表）」の `uts-<term>` 列（リンク）を参照する。
