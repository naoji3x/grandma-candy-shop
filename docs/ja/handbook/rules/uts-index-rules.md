...
対象単位（`uts-<term>`）の一覧は、**6.5 の「対象単位一覧（統合表）」** で管理します（同一内容の重複記載はしません）。

### 7.3. 単体テストの境界（モック/スタブ方針）（必須）
...
対象単位の定義・TPC の割当・個別仕様リンクは 6.5 の「対象単位一覧（統合表）」で一括管理します（重複記載しません）。

### 7.5. 対象単位一覧（統合表）（必須）

運用上の更新漏れを防ぐために、**対象単位の定義（6.2）・分配方針（6.4）・個別仕様一覧（6.7）を 1 表に統合**し、ここを単一ソース（Single Source of Truth）とします。
...
- 個別仕様一覧は、**6.5 の「対象単位一覧（統合表）」の `uts-<term>` 列（リンク）**で管理します。
- 一覧の重複記載はせず、6.5 の統合表が最新版であることを保証します（命名ゆれ禁止）。
...
対象単位（`uts-<term>`）の一覧は、「対象単位一覧（統合表）」（8.6）で管理する。

### 9.4. 単体テストの境界（モック/スタブ方針）
