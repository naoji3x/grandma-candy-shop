...
本章 3.1〜3.4 は本ルールの **規範（Normative）** であり、3.5 以降は理解補助のための **参考情報（Informative）** とする。
...
1. **仕様 Index** (`xxs-index`)
   - TPC を当該レベルの責務として解釈し、分配方針（境界・対象単位・合格基準）を固定する
   - 個別仕様（`xxs-<term>`）への入口（ナビゲーション）となる
     ※ ケースは書かない
...
3. **設計 Index** (`xxd-index`)
   - 共通のケース構成ルール（観点×条件→ケース）と、準備・実行・証跡の標準を定義する

4. **設計 個別** (`xxd-<term>`)
   - 観点（TP）×条件（TC）を組み合わせてテストケースを具体化し、実行可能粒度まで落とす

※ xx はテストレベル略号（ut/it/et/st/at）を表す。s は Specification、d は Design を表す。
...
1. **「保証の話か？」**
   - Yes → `xxs-index` / `xxs-<term>`（UTS/ITS/ETS/STS/ATS）
   - No → 次へ
...
3. **「具体値・ケース展開か？」**
   - Yes → `xxd-index` / `xxd-<term>`（UTD/ITD/ETD/STD/ATD）
   - No → 上位（TSP）を疑う
...
| UTD, ITD, ETD, STD, ATD に 保証範囲（What）の説明・根拠がある | 重複・更新漏れ   |
| 同じ表が複数文書に存在                                        | 破綻の温床       |
