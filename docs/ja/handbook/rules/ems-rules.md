...
ESIL 側の `spec_ref: ems-...` は、このEMSのID（およびファイル名）と対応させます。
...
- ファイル名: `ems-<短い英小文字ハイフン>.yaml`
- ESILからの参照: `spec_ref: ems-...`（例: `ems-stock-changed`）
...
> - `x-spec-meta` を必ず含め、`id`（`ems-...`）、`type`（`message`）、`status`（`draft`/`ready`/`deprecated`）を設定してください。
