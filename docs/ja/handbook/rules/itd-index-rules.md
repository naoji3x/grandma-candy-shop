...
`itd-index` に **共通方針・共通基準・ナビゲーション**を集約し、設計〜実装〜レビューの迷いと抜け漏れを減らします。

- 扱う: テスト単位/ケース分解/データ/実行/判定の共通方針、`itd-<term>` への入口
- 扱わない: 個別ケースの網羅、具体値の列挙、テストコード全文
- トレースは Frontmatter の `based_on` を起点にする（大量列挙は上位成果物へ集約）
- 曖昧表現は禁止（対象/境界/基準/観測点が判定できる言葉で書く）
- 未定義プロパティ追加は禁止（`additionalProperties: false`）
