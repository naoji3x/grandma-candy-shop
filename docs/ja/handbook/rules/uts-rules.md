...
flowchart TD
  ...
  UTS["**uts-&lt;term&gt;<br>単体テスト仕様-個別仕様**"]
  UTDIndex["uts-index<br>単体テスト設計-全体構成"]
  UTD["uts-&lt;term&gt;<br>単体テスト設計-個別設計"]
  TC["テストコード<br>JUnit 等"]

  TSP --> TPC --> UTSIndex --> UTS --> UTD --> TC
  TSP --> UTSIndex
  TPC --> UTS
  UTSIndex --> UTDIndex
  UTDIndex --> UTD
...
ファイル名は`uts-010-単体テスト仕様-<用語集term>.md` 等、プロジェクト内で一意になるように命名します。
...
- 全体構成（`uts-index`）との差分・特記事項があれば明示します。
