...
ESIL 側の `spec_ref: efes-...` は、このEFESのID（およびファイル名）と対応させます。
...
- ファイル名: `efes-<短い英小文字ハイフン>.yaml`
- ESILからの参照: `spec_ref: efes-...`（例: `efes-orders`）
...
| キー       | 説明                                                   | 必須 |
| ---------- | ------------------------------------------------------ | ---- |
| id         | ファイル仕様ID（`efes-...`）、ESILの `spec_ref` と対応 | ○    |
| type       | `file` 固定                                            | ○    |
