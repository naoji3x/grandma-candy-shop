...
`utd-<term>`（単体テスト設計: 個別設計）は、以下を明文化し、設計〜実装〜レビューの迷いと抜け漏れを減らします。

- どの UTS（`uts-<term>`）を、どの粒度でケースへ落とすか
- 代表ケースの作り方（総当たり回避の基準）
- データ準備・モック/スタブの設計方針
- テストコード構成・命名・配置の方針
- 実行方法・結果参照（エビデンス）の方針
...
- 個別 UTD（`utd-<term>`）は **単体テスト設計（UTD）の個別化**であり、独立した方針文書ではありません。
  - 必ず `utd-index` を前提にします。

- 1ファイル = 1論理対象（原則）です。
  - 対象名は **用語集（GL）の term** を採用し、将来のリンク切れを防ぎます。

- 個別 UTD は **テストケース設計**までを担います。
...
ファイル名は`utd-010-単体テスト設計-<term>.md` 等、プロジェクト内で一意になるように命名します。

- 例： `utd-010-単体テスト設計-在庫.md`, `utd-010-単体テスト設計-注文.md`

> `<term>` は **用語集（GL）に定義された論理名**を使用します。

### 4.2. ID 規則

`id`: `utd-<term>` 形式（小文字・ハイフン）
...
個別 UTD（`utd-<term>`）は以下の見出し構成を **必ず守ります**。
