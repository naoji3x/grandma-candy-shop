...
- TSP は原則 **`tsp-overview` 1本** に統一します。分割ルールは [セクション 3](#3-ファイル命名id規則) 参照。
...
| 前提（下位で保証済み）     | 入口条件として、前フェーズで担保されていること                       |
...
外部I/F（他システム連携）の“どこまで確認するか”を必ず言語化します（例）:

- 導通: ネットワーク経由で到達できる/認証できる/基本的なリクエストが通る
- 契約（コントラクト）: スキーマ、必須項目、コード値、エラー形式、タイムアウト/リトライ、冪等性キー等
- データ内容: I/Fデータの生成根拠・変換ルール・丸め/桁/NULLの扱い（主に自システム側責務）
- 業務一貫性: 外部システムを含めたE2Eの整合性（範囲を合意して限定するのが現実的）

記載例（合意文として書く）:

- 外部I/Fとのテストは、外部結合で **契約と代表エラー**、総合/受入で **導通＋主要シナリオのスモーク** までを対象とする。
