...
- `id`: `tsp-overview`, `tsp-regression` のように小文字ハイフン形式。
  ...
  - 分割する場合は、方針の対象範囲が分かる ID にします。<br>例: `tsp-nfr-testing`（非機能試験方針）, `tsp-regression`（回帰方針）
...
- 業務: 販売、在庫、発注・入荷、棚卸、売上・在庫レポート
- I/F: 外部決済、帳票出力（CSV/PDF 等）
